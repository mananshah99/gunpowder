...
viewer = neuroglancer.Viewer(voxel_size=(4, 4, 40))
...
#         )
# else:
#     with h5py.File("/groups/turaga/home/grisaitisw/src/gunpowder/examples/fib25/tstvol-520-1-h5_y0_x0_xy0_angle000.0.h5", "r") as f:
#         for key in ("/volumes/raw", "/volumes/labels/neuron_ids", "/volumes/labels/mask"):
#             dtype = dtypes.get(key, np.dtype("float32"))
#             if array.dtype is not dtype:
#                 print("recasting", key, "from", array.dtype, "to", dtype)
#                 new_array = array.astype(dtype)
...
#                 array = new_array
#             viewer.add(
#                 f[key][:].astype(dtypes[key]),
#                 name="original/{}".format(key),
#                 offset=(0, 0, 0),
#                 voxel_size=(8, 8, 8),
#             )
